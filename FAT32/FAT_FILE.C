...
void* fl_fopen(const char *path, const char *mode)
{
	...
	// "a+" Open a file for reading and appending. All writing operations are performed at the end of the file, protecting the previous content to be overwritten. You can reposition (fseek, rewind) the internal pointer to anywhere in the file for
...
int fl_fwrite(const void * data, int size, int count, void *f )
{
	...
	while (bytesWritten < length)
	{
		...
		memcpy((unsigned char*)(file->file_data.sector + offset), (unsigned char*)(buffer + bytesWritten), (size_t)copyCount);
